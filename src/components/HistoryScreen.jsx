...
const HistoryScreen = ({ setSeeLogs, setPhoneNumber, handleCall }) => {
  ...
  return (
    <div className="flex flex-col h-full md:p-0 p-6">
      {/* Header */}
      <div className="flex items-center justify-between p-2 border-b border-border bg-transparent md:bg-muted/60">
        <div className="flex items-center gap-2 sm:gap-3">
          <button
            onClick={handleGoBack}
            className="md:inline-flex hidden items-center justify-center w-8 h-8 sm:w-9 sm:h-9 rounded-full bg-transparent md:bg-muted hover:bg-accent transition-colors text-muted-foreground hover:text-primary"
            aria-label="Back"
          >
      ...
      {/* Footer hint */}
      {history.length > 5 && (
        <div className="bg-muted/70 sm:block hidden border-t border-border px-3 sm:px-4 md:px-6 py-2 sm:py-3">
