...
function Dashboard() {
  ...
  const fetchNextLead = useCallback(async () => {
    ...
    try {
      ...
      console.log('[fetchNextLead] response:', {
        success: response.data?.success,
        hasLead: !!nextLead,
        leadId: nextLead?.leadId,
        leadState: nextLead?.leadState,
        lastDialedStatus: nextLead?.lastDialedStatus,
        lockToken: !!nextLead?.lockToken,
      });
  ...
  return (
    <div className="flex-1 overflow-y-auto">
      ...
      {(() => {
        const shouldShowRingtone = ringtone?.length > 0 && ringtone[0]?.campaign === userCampaign;
          ...
          {dashboardCards.map((card) => {
            ...
            return (
              <button
                ...
                type="button"
                onClick={
                  activeMainTab === 'leads' && previewLeadMode
                    ? undefined
                    : () => setActiveMetricFilter((prev) => (prev === card.key ? 'all' : card.key))
                }
