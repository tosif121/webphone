...
export default function AgentCallData() {
  ...
  const fetchCallData = async () => {
    ...
    try {
      ...
      const { data } = await axios.post(`https://devapp.iotcom.io/agentcallData`, payload, {
        headers: {
  ...
  const handlePlayAudio = async (bridgeID) => {
    ...
    try {
      ...
      const response = await axios.get(`https://devapp.iotcom.io/recording/recording${bridgeID}.wav`, {
        headers: {
          Authorization: `Bearer ${tokenDetails.token}`,
        },
        responseType: 'blob',
